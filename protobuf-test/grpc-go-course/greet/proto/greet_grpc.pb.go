...
const (
	...
	GreetService_LogGreet_FullMethodName       = "/greet.GreetService/LogGreet"
...
type GreetServiceClient interface {
	...
	LogGreet(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[GreetRequest, GreetResponse], error)
...
func (c *greetServiceClient) LogGreet(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[GreetRequest, GreetResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[1], GreetService_LogGreet_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[GreetRequest, GreetResponse]{ClientStream: stream}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type GreetService_LogGreetClient = grpc.ClientStreamingClient[GreetRequest, GreetResponse]
...
type GreetServiceServer interface {
	...
	LogGreet(grpc.ClientStreamingServer[GreetRequest, GreetResponse]) error
...
func (UnimplementedGreetServiceServer) LogGreet(grpc.ClientStreamingServer[GreetRequest, GreetResponse]) error {
	return status.Error(codes.Unimplemented, "method LogGreet not implemented")
}
...
func _GreetService_LogGreet_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).LogGreet(&grpc.GenericServerStream[GreetRequest, GreetResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type GreetService_LogGreetServer = grpc.ClientStreamingServer[GreetRequest, GreetResponse]
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "LogGreet",
			Handler:       _GreetService_LogGreet_Handler,
			ClientStreams: true,
		},
		{
