...
def load_pipeline(args):
    print(f"Loading Stable Diffusion model: {args.model}", file=sys.stderr, flush=True)
    ...
    if args.safetensors and os.path.exists(args.safetensors):
        ...
        if model_type == "sdxl":
            ...
            print("✓ Loaded SDXL from single file", file=sys.stderr)
        elif model_type == "sd3":
            ...
            print("✓ Loaded SD3 from single file", file=sys.stderr)
        else:  # SD 1.5 / 2.1
            ...
            print(f"✓ Loaded {model_type.upper()} from single file", file=sys.stderr)
    ...
    # Load from HuggingFace Hub
        ...
        try:
            ...
            print(
                "⚠ FP16 variant not found, loading standard weights", file=sys.stderr
            )
            pipe = AutoPipelineForText2Image.from_pretrained(
                args.model,
                torch_dtype=dtype,
        ...
        if hasattr(pipe.tokenizer, "model_max_length"):
            ...
            if model_type == "sd15" or model_type == "sd2":
                if current_max_length != 77:
                    ...
                    print(
                        "✓ Tokenizer max length set to 77 (SD standard)",
                        file=sys.stderr,
                    )
            else:
    ...
    if args.low_vram:
        if args.sequential_offload:
            ...
            print(
                "✓ Sequential CPU offload enabled (ultra low VRAM mode)",
                file=sys.stderr,
            )
        else:
    ...
    if args.compile:
        ...
        if hasattr(pipe, "unet"):
            ...
            print(
                "✓ Transformer compiled (first generation will be slow)",
                file=sys.stderr,
            )
    ...
    print(
        f"✓ Recommended resolution: {get_recommended_resolution(model_type)}",
        file=sys.stderr,
    )
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        print(
            "WARNING: CUDA not available, generation will be very slow!",
            file=sys.stderr,
        )
    ...
    parser.add_argument(
        "--safetensors", default="", help="Path to safetensors file (Civitai models)"
    )
    ...
    parser.add_argument(
        "--prompts-data", required=True, help="JSON string of prompt data"
    )
    ...
    try:
        ...
        print(
            f"\n🎨 Starting generation of {len(prompts_data)} images...\n",
            file=sys.stderr,
        )
        ...
        for i, p_data in enumerate(prompts_data):
            ...
            print(
                f"[{i+1}/{len(prompts_data)}] Generating: {prompt[:60]}...",
                file=sys.stderr,
            )
            print(
                f"  Resolution: {args.width}x{args.height}, Steps: {args.steps}, Seed: {seed}, CFG: {args.guidance_scale}",
                file=sys.stderr,
            )
            gen_start = time.time()
